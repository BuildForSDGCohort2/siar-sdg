    ...
    function init(){
        ...
        if($check && mysqli_num_rows($check) > 0){
            ...
            if(!in_array('user',$tables)){
                ...
                $sql = "create table user(id bigint auto_increment primary key, user_id varchar(16) not null, first_name varchar(255) not null, middle_name varchar(255),last_name varchar(255) not null, password varchar(255) not null,email varchar(255),phone varchar(255) not null, dob bigint, ranking int(2), position varchar(255),date_created bigint,date_modified bigint,last_login bigint)";
                $q = $con->query($sql);
                if($q){
                    $sql2 = "insert into user(user_id,first_name,last_name,password,email,phone,date_created,date_modified) values('admin','System','Administrator','".password_hash('NoPassword1',PASSWORD_BCRYPT)."','support@neelansoft.co.tz','+255784086726',".time().",".time().")";
                }
            }
            else{
                $sql2 = "insert into user(user_id,first_name,last_name,password,email,phone,date_created,date_modified) values('admin','System','Administrator','".password_hash('NoPassword1',PASSWORD_BCRYPT)."','support@neelansoft.co.tz','+255784086726',".time().",".time().")";
